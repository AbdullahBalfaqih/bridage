...
export function LikeButton({ projectId }: { projectId: string }) {
  ...
  const favoriteRef = useMemoFirebase(
    () => (user && projectId ? doc(firestore, 'users', user.uid, 'favorites', projectId) : null),
    [firestore, user, projectId]
  );
  
  const { data: favoriteDoc } = useDoc(favoriteRef);
  ...
  return (
    ...
    >
        <input id={id} className="checkbox" type="checkbox" checked={isFavorite} readOnly />
